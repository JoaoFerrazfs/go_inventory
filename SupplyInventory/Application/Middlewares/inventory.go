...
import (
	...
	invRepo "go_inventory/SupplyInventory/Domain/contracts/repositories/Inventory"

	"github.com/gin-gonic/gin"
)

type InventoryMiddleware struct {
    InventoryRepository invRepo.InventoryRepository
}

func NewInventoryMiddleware(repo invRepo.InventoryRepository) *InventoryMiddleware {
...
func (m *InventoryMiddleware) Handler() gin.HandlerFunc {
    return func(c *gin.Context) {
        ...
        if appErr != nil || !exists {
            c.AbortWithStatusJSON(422, errors.NewAppError("Inventory not found", 422))
            return
        }
