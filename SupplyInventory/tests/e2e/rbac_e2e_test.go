...
func TestRBAC_E2E(t *testing.T) {
       ...
       dsn := "root:root@tcp(db-test:3306)/inventory_test?charset=utf8mb4&parseTime=True&loc=Local"
       ...
       // Hash de senha igual ao backend (ajuste se necessário)
       adminPass := "SenhaForte123"
       passHashBytes, err := bcrypt.GenerateFromPassword([]byte(adminPass), bcrypt.DefaultCost)
       if err != nil {
       ...
       _, err = db.Exec(`INSERT INTO user_entities (name, email, password, role) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE role=VALUES(role), password=VALUES(password)`,
               "Admin E2E", "admin_e2e@test.com", passHash, "admin")
       if err != nil {
